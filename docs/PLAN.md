...
| `next:<idx>` | *(в Workers-версии `callback_data` несёт индекс вопроса, для которого показана клавиатура; голое `next` со старых кнопок работает как раньше.)* Если `idx` не совпадает с `current_question` — клавиатура устарела (мама прислала два голосовых подряд и на экране две клавиатуры): отредактировать на *«✅ Этот вопрос уже пройден»* и ничего не менять. Ответ Telegram «message is not modified» — повторный тап, молчание. Иначе — отредактировать на *«✅ Идём дальше ❤️»*; `current_question += 1`; очистить part-массивы; `waiting_for_voice=true`; `last_activity=now`; сохранить; отправить новый вопрос (если вопросы кончились — сработает финальное сообщение). **На Диск здесь ничего не сохраняется — это уже произошло при получении каждого голосового.** |
...
Переносим осознанно, чинить — отдельным решением. **Статус в Workers-версии:** п.1 и п.4 починены (`part_seq` сбрасывается только при смене вопроса: `next`, `confirm_reset`, `/jump` на другой номер; любой повторный `/start` — ветка «С возвращением»). П.2 и п.3 оставлены как есть.
