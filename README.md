...
> **Актуальная версия бота — на Cloudflare Workers** (`src/`, `wrangler.json`, `schema.sql`).
> Как её развернуть и проверить — в [docs/PLAN.md](docs/PLAN.md), Часть II.
> Ниже — инструкция для старой Python-версии (`bot.py`, Railway); она оставлена как справка.
